	...
	protected $_languages;
 	function __construct()
 	{
		...
		$this->page = basename(__FILE__, '.php');
	 	$this->displayName = $this->l('BxSlider by Liamka');
...
*/
	...
	{
		$this->context->controller->addCSS($this->_path.'css/style_nivo.css');
	}
        ...
        $isoUser = Language::getIsoById(intval($cookie->id_lang));
 	 	if (file_exists(dirname(__FILE__).'/links.xml'))
 	 		if ($xml = simplexml_load_file(dirname(__FILE__).'/links.xml'))
 	 		{
				...
				return $this->display(__FILE__, 'bxslider.tpl');
			}
		return false;
 	}
}
